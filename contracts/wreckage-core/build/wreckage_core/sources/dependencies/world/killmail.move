...
// === Accessors (added for wreckage-protocol integration) ===
public fun key(km: &Killmail): TenantItemId { km.key }
public fun killer_id(km: &Killmail): TenantItemId { km.killer_id }
public fun victim_id(km: &Killmail): TenantItemId { km.victim_id }
public fun kill_timestamp(km: &Killmail): u64 { km.kill_timestamp }
public fun loss_type(km: &Killmail): LossType { km.loss_type }
public fun solar_system_id(km: &Killmail): TenantItemId { km.solar_system_id }

// === Test-only Helpers ===
#[test_only]
public fun destroy_for_testing(km: Killmail) {
    let Killmail { id, .. } = km;
    id.delete();
}

#[test_only]
public fun create_test_killmail(
    ...
    Killmail {
        ...
        killer_id,
        victim_id,
        reported_by_character_id: killer_id,
        kill_timestamp,
        loss_type: LossType::SHIP,
        solar_system_id: killmail_key,
    }
}
