...
fun init(ctx: &mut TxContext) {
    ...
    // M-8: AuctionRegistry no longer holds its own AuctionConfig copy
    // Single source of truth is ProtocolConfig.auction_config
    auction::create_and_share_auction_registry(ctx);

    // ValuationRegistry → shared (item price oracle for EVE integration)
